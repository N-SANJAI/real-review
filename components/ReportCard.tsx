...
export default function ReportCard({ report, product, className = "" }: Props) {
  const scoreColor =
    report.score >= 75
  ...
  return (
    <div className={`space-y-6 ${className}`.trim()}>
      <div className="flex flex-col gap-5 rounded-2xl border border-slate-200 bg-white/95 p-6 shadow-lg shadow-cyan-100/60 sm:flex-row sm:items-start sm:justify-between dark:border-slate-800 dark:bg-slate-900/95 dark:shadow-cyan-950/30">
        <div className="min-w-0 flex-1">
          <p className="mb-2 text-xs uppercase tracking-widest text-cyan-600 dark:text-cyan-400">
            Verdict · <span className="break-words">{product}</span>
          </p>
        ...
        <div className="w-full shrink-0 rounded-xl bg-slate-100 px-4 py-3 text-center sm:w-auto dark:bg-slate-800">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900/60 dark:bg-emerald-950/30">
          <p className="mb-3 text-xs uppercase tracking-widest text-emerald-600 dark:text-emerald-300">
            Pros
          </p>
        ...
        <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 dark:border-rose-900/60 dark:bg-rose-950/30">
          <p className="mb-3 text-xs uppercase tracking-widest text-rose-600 dark:text-rose-300">
            Cons
          </p>
          <ul className="space-y-2">
      ...
      {report.red_flags.length > 0 && (
        <div className="rounded-2xl border border-amber-300 bg-amber-50 p-5 dark:border-amber-900/70 dark:bg-amber-950/30">
          <p className="mb-3 text-xs uppercase tracking-widest text-amber-700 dark:text-amber-300">
            ⚠ Red Flags
          </p>
          <ul className="space-y-2">
            {report.red_flags.map((flag, i) => (
              <li key={i} className="break-words text-sm text-amber-900 dark:text-amber-100">
                {flag}
              </li>
            ))}
          </ul>
